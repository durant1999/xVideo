...
class DurationGuardTests(unittest.TestCase):
    def test_default_max_duration_is_twenty_minutes(self):
        self.assertEqual(DEFAULT_CONFIG["video"]["max_duration_seconds"], 1200)

    def test_enforce_duration_limit_rejects_over_limit_video(self):
        with self.assertRaises(PipelineError) as ctx:
            enforce_duration_limit(1201.0, 1200, video_path="/tmp/long.mp4")

        message = str(ctx.exception)
        self.assertIn("20m 1s", message)
    ...
    def test_enforce_duration_limit_allows_equal_and_disabled_limits(self):
        enforce_duration_limit(1200.0, 1200)
    ...
    def test_cli_override_takes_precedence_over_config(self):
        ...
        self.assertEqual(max_duration_seconds_for(args, {"max_duration_seconds": 1200}), 60)

    def test_negative_cli_limit_is_rejected(self):
        args = argparse.Namespace(max_duration_seconds=-1)

        with self.assertRaises(PipelineError):
            max_duration_seconds_for(args, {"max_duration_seconds": 1200})

    def test_format_duration(self):
        self.assertEqual(format_duration(59.4), "59s")
        self.assertEqual(format_duration(1200), "20m 0s")
